...
class ELFManip(object):
    ...
    def _init_phdrs(self):
        ...
        for s in self.elf.iter_segments():
            phdrs["entries"].append(
                Segment(
                    ENUM_P_TYPE_BASE[s["p_type"]],
                    s["p_offset"],
                    s["p_vaddr"],
                    s["p_paddr"],
                    s["p_filesz"],
        ...
        logger.debug(
            "Copied %d program headers from %s", len(phdrs["entries"]), self.filename
        )
        return phdrs
    ...
    def _init_shdrs(self):
        ...
        for s in self.elf.iter_sections():
            ...
            shdrs["entries"].append(
                Section(
                    s["sh_name"],
                    ENUM_SH_TYPE_BASE[s["sh_type"]],
                    s["sh_flags"],
                    s["sh_addr"],
                    s["sh_offset"],
                    s["sh_size"],
        ...
        logger.debug(
            "Copied %d section headers from %s", len(shdrs["entries"]), self.filename
        )
        return shdrs

    def _init_ehdr(self):
        return {
            "e_ident": self.elf.e_ident_raw,
    ...
    def dump_ehdr(self):
        ...
        return self.ehdr["e_ident"] + struct.pack(
            "<HHI%s%s%sIHHHHHH" % (size, size, size),
            self.ehdr["e_type"],
            self.ehdr["e_machine"],
            self.ehdr["e_version"],
            self.ehdr["e_entry"],
    ...
    def relocate_phdrs(
        self, custom_offset=None, new_size=None, segment=None, use_methods=None
    ):
        """
        ...
        if custom_offset is not None:
            if new_size is None:
                logger.warning(
                    "You must specify the size of the relocated program headers"
                )
                return None
    ...
    def _phdr_hack1(self):
        ...
        logger.debug(
            "Sections on either side of the segment padding: [%s, %s]",
            section_before_padding.name,
            section_after_padding.name,
        )

        free_space_start = (
            section_before_padding["sh_offset"] + section_before_padding["sh_size"]
        )
        free_space_size = section_after_padding["sh_offset"] - free_space_start

        logger.debug(
            "Found 0x%x bytes of padding at offset 0x%x. Thats enough for %d entries of size %d each.",
            free_space_size,
            free_space_start,
            free_space_size // self.ehdr["e_phentsize"],
            self.ehdr["e_phentsize"],
        )

        minimum_required_space = (
            self.ehdr["e_phnum"] + self.num_adtl_segments
        ) * self.ehdr["e_phentsize"]
        if free_space_size >= minimum_required_space:
            ...
            if not self.unsafe_try_hard:
                empty = True
                self._file.seek(
                    section_before_padding["sh_offset"]
                    + section_before_padding["sh_size"]
                )
                ...
                if not empty:
                    ...
                    raise BadELF(
                        "Padding is not empty. Use unsafe_try_hard if you want to proceed."
                    )

            self._update_phdr_entry(free_space_start, free_space_size)
            return True
        logger.info(
            "Method 1 failed - there is not enough space between segments to fit the new program headers"
        )
        return False
    ...
    def _phdr_hack2(self):
        ...
        if self.num_adtl_segments != 1:
            logger.info(
                "Method 2 does not apply: Would only provide one additional segment"
            )
            return False
    ...
    def _phdr_hack3(self):
        ...
        interp_index = (
            1
        )  # hardcoded and expected location of the .interp section header entry
        ...
        if num_new_phdr_entries < self.num_adtl_segments:
            logger.warning(
                "Method 3 failed: Not enough space for %d additional phdrs",
                self.num_adtl_segments,
            )
            return False
        ...
        logger.debug(
            "resulted in room for %d additional program headers", num_new_phdr_entries
        )
        ...
        for i in range(num_freed_sections - 1, -1, -1):
    ...
    def _update_phdr_entry(self, new_base, max_size, segment=None):
        ...
        for p in self.phdrs["entries"]:
            ...
            if p.p_type == ENUM_P_TYPE_BASE["PT_PHDR"]:
                logger.debug(
                    "Updating the PHDR segment to new offset: 0x%x", self.phdrs["base"]
                )
                ...
                if self.arch == "x64":
                    p.p_filesz = (
                        len(self.phdrs["entries"]) * 56
                    )  # 56 bytes each for 64-bit
                else:
                    p.p_filesz = (
                        len(self.phdrs["entries"]) * 32
                    )  # 32 bytes each for 32-bit
        ...
        # more specifically, we assume that simply *extending* the phdrs in place will grow into a different segment
            ...
            if not found:
                logger.warning(
                    "problem finding LOAD segment containing new phdr location"
                )
    ...
    def add_section(self, section, segment=None):
        ...
        if segment is not None and not isinstance(segment, CustomSegment):
            raise TypeError("segment must be a CustomSegment instance")

        section.arch = (
            self.arch
        )  # Override initial value so that custom section's arch matches the binary
        if segment is not None:
    ...
    def remove_shdr_entry(self, sh_num):
        ...
        for section in self.shdrs["entries"]:
            if section.sh_type == ENUM_SH_TYPE_BASE["SHT_HASH"]:
                ...
                ENUM_SH_TYPE_BASE["SHT_REL"],
                ENUM_SH_TYPE_BASE["SHT_RELA"],
            ]:
                ...
                ENUM_SH_TYPE_BASE["SHT_SYMTAB"],
                ENUM_SH_TYPE_BASE["SHT_DYNSYM"],
            ]:
    ...
    def add_segment(self, segment):
        ...
        segment.arch = (
            self.arch
        )  # Override initial value so that custom segment's arch matches the binary
        ...
        if len(self.phdrs["entries"]) < self.phdrs["max_num"]:
            ...
            logger.error(
                "Cannot add another section. Not enough room in the program headers to add another segment"
            )
            return None
        self.ehdr["e_phnum"] += 1
    ...
    def write_new_elf(self, outfile):
        if outfile == self.filename:
            logger.error(
                "Must specify a different file destination than the original ELF"
            )
            return
        ...
        with open(outfile, "wb") as f:
            ...
            if added_segment:
                ...
                logger.info(
                    "Appending %d section header entries (%d + %d)",
                    len(self.shdrs["entries"]) + len(self.custom_sections),
                    len(self.shdrs["entries"]),
                ...
                logger.debug(
                    "Writing program headers to offset 0x%x", self.phdrs["base"]
                )
            ...
            for i, section in enumerate(self.shdrs["entries"]):
                for new_bytes, section_offset in section.buffered_writes:
                    logger.debug(
                        "Patching in user-defined bytes for section %d (%s)",
                        i,
                        section.sh_name,
                    )
    ...
    def write_to_section(self, section, new_bytes, offset=0):
        if isinstance(section, Section):
            ...
            logger.warning(
                "Use section.contents=updated_contents to change the contents of a CustomSection"
            )
        else:
...
class Section(object):
    ...
    def get_current_contents(self):
        ...
        for new_bytes, offset in self.buffered_writes:
            current_contents = (
                current_contents[0:offset]
                + new_bytes
                + current_contents[offset + len(new_bytes) :]
            )
    ...
    def __str__(self):
        return (
            "Name: 0x%x, Type: %s, Flags: %s, Addr: 0x%08x, Offset: %s, Size: %s bytes, Link: %s, Info: %s, Align: %d, EntSz: %d"
            % (
                self.sh_name,
                self.sh_type,
                self.sh_flags,
...
class Segment(object):
    ...
    def __str__(self):
        return (
            "Type: 0x%x, Offset: 0x%08x, Vaddr: 0x%08x, Paddr: 0x%08x, Filesize: 0x%08x, Memsize: 0x%08x, Flags: 0x%08x, Align: %d"
            % (
                self.p_type,
                self.p_offset,
                self.p_vaddr,
    ...
    def finalize(self):
        ...
        for section in self.sections:
            if not section.is_defined():
                logger.error(
                    "Section to be loaded at 0x%x is not fully initialized!"
                    % section.sh_addr
                )
                return
...
def pad_to_modulus(f, modulus, padding_bytes=b"\x00", pad_if_aligned=False):
    ...
    f.write(
        padding_bytes * (padding_len // len(padding_bytes))
        + padding_bytes[: padding_len % len(padding_bytes)]
    )
